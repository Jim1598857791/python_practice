...
def writeCSV(a, b):
    try:
        if b == '1':
            ...
            csvFile = open('result\未找到.csv', 'a+', newline='')  # 设置newline，否则两行之间会空一行
...
# East=[]
err=[]
for row in csv_reader_Sum_English:
    x = csv_reader_Sum_English.index(row)
    y = [csv_reader_Sum[x],]
    ...
    #     East.append(x)
    #     writeCSV(y,'East')
    elif csv_reader_Sum_Name[x] in csv_reader_1 :
        print(row,"英文名查找失败;",csv_reader_Sum_Name[x],",中文名成功,在1号")
        one.append(x)
        writeCSV(y,'1')
    elif csv_reader_Sum_Name[x] in csv_reader_2 :
        print(row,"英文名查找失败;",csv_reader_Sum_Name[x],"，中文名成功，在2号")
        two.append(x)
        writeCSV(y,'2')
    elif csv_reader_Sum_Name[x] in csv_reader_3 :
        print(row,"英文名查找失败;",csv_reader_Sum_Name[x],"，中文名成功，在3号")
        three.append(x)
        writeCSV(y,'3')
    elif csv_reader_Sum_Name[x] in csv_reader_Out :
        print(row,"英文名查找失败;",csv_reader_Sum_Name[x],"，中文名成功，在校外")
        out.append(x)
        writeCSV(y,'out')
    ...
    #     Hotel.append(x)
    #     writeCSV(y,'hotel')
    elif csv_reader_Sum_Name[x] in csv_reader_Hotel :
        print(row,"中文名查找失败;",csv_reader_Sum_Name[x],"，英文名成功，在宾馆")
        Hotel.append(x)
        writeCSV(y,'hotel')
    else:
        print(row,"中文名查找失败;",csv_reader_Sum_Name[x],"，英文名失败")
        err.append(x)
        writeCSV(y,'err')
